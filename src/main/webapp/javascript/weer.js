...
/*
 * LES5 VARIANT function showWeather(lat, lon, city) {
 ...
 * "&lon=" + lon + "&units=metric&appid=31448cd3d20637809d3a55949c0a6f98",
 * function (data) { console.log(data); $("#temp").html(data.main.temp + "°C");
 * $("#lv").html(data.main.humidity + "%"); $("#ws").html(data.wind.speed * 3.6 + "
 * km/u"); $("#wr").html(windrichting(data.wind.deg));
...
function showWeather(lat, long, city) {
	...
	function get() {
		var getURL = "http://api.openweathermap.org/data/2.5/weather?lat="
				+ lat + "&lon=" + long
				+ "&units=metric&APPID=31448cd3d20637809d3a55949c0a6f98";
...
function loadCountries() {
				...
				$.ajax({
							url : "/firstapp/restservices/countries/GETall",
							type : "GET",
							...
							success : function(result) {
								$.each(result, function(index, object) {
									...
									var String = "<tr id='row" + index + "'>"
											...
											+ "<td>" + object.population
											+ "</td>" + "</tr>";
									countryTable.append(String);
									$("#row" + index).click(
											function() {
												showWeather(object.lat,
														object.lon,
														object.capital);
											});
...
function windrichting(degrees) {
	if (degrees !== undefined) {
		...
		var arr = [ "N", "NNE", "NE", "ENE", "E", "ESE", "SE", "SSE", "S",
